...
def test_unique_active_request_per_asn_network(db_session: Session) -> None:
    ...
    with pytest.raises(DuplicateActiveRequestError):
        repo.create_pending_request(user_id=user_id, asn=64512, zt_network_id=network_id)
    ...
    replacement = repo.create_pending_request(user_id=user_id, asn=64512, zt_network_id=network_id)
